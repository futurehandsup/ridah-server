...
var NoticeOwnerSchema = new Schema({
    noticeTitle : {
      type: String,
      index : true,         // 보조 index
...
NoticeOwnerSchema.post('update', function(result) {
  this.update({_id  : result.id },{ $set: { updated_at: new Date() } });
});

NoticeOwnerSchema.set('toJSON',{ getters : true , virtuals : true});
...
mongoose.model('NoticeOwner', NoticeOwnerSchema);
